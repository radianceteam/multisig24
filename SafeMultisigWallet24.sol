...
/// @title Multisignature wallet with transaction lifetime 24 hour
/// @author Tonlabs (https://tonlabs.io) updated by Radianceteam
contract MultisigWallet is IAccept {
    ...
    uint64  constant EXPIRATION_TIME = 86400; // lifetime is 24 hour
    ...
    */
    ...
    /// @dev Generates new id for object. Updated from tvm.transLT() to tx.timestamp
    ...
    /// @dev A  method for accepting incoming funds. Generates
    ...
    function _removeExpiredTransactions() inline private {
        ...
        while (needCleanup && i < MAX_CLEANUP_TXNS) {
            ...
            optional(uint64, Transaction) trn = m_transactions.next(trId);
            if (trn.hasValue()) {(trId, txn) = trn.get();success = true;} else {success = false;}
    ...
    function getTransactions() public view returns (Transaction[] transactions) {
        ...
        Transaction txn;
        bool success;
        if (tr.hasValue()) {(id, txn) = tr.get();success = true;} else {success = false;}
        while (success) {
            ...
            optional(uint64, Transaction) trn = m_transactions.next(id);
            if (trn.hasValue()) {(id, txn) = trn.get();success = true;} else {success = false;}
        }
    }
    ...
    function getTransactionIds() public view returns (uint64[] ids) {
        ...
        optional(uint64, Transaction) tr = m_transactions.min();
        if (tr.hasValue()) {(trId, ) = tr.get();success = true;} else {success = false;}
        while (success) {
            ids.push(trId);
            // Update mapping processing
            // (trId, , success) = m_transactions.next(trId);
            optional(uint64, Transaction) trn = m_transactions.next(trId);
            if (trn.hasValue()) {(trId, ) = trn.get();success = true;} else {success = false;}
        }
    }
    ...
    function getCustodians() public view returns (CustodianInfo[] custodians) {
        ...
        optional(uint256, uint8) cst = m_custodians.min();
        uint256 key;
        uint8 index;
        bool success;
        if (cst.hasValue()) {(key, index) = cst.get();success = true;} else {success = false;}
        while (success) {
            custodians.push(CustodianInfo(index, key));
            // Update mapping processing
            // (key, index, success) = m_custodians.next(key);
            optional(uint256, uint8) cstn = m_custodians.next(key);
            if (cstn.hasValue()) {(key,index) = cstn.get();success = true;} else {success = false;}
        }
    }
